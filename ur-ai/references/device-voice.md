...
分别对齐 ASR 完成、识别队列消费、模型首文本、TTS 首音。上一轮结束后若仍同步
调用画像整理模型，会阻塞下一轮；复用平台后台轮次保存和记忆提取，不新增第二套任务。
当前会话 history、跨 session 最近轮次和长期画像必须分开验收，异步画像不保证
新建 session 后数百毫秒立即可见，不能用同会话三轮通过替代跨会话记忆测试。

Runtime 可靠 TTS 在首段有效文本后才启动首帧预算；模型或工具等待不能耗尽合成预算，
也不能单纯延长超时冒充性能优化。回归包含慢模型（超过五秒才产文本）、空白、取消、
真实图片音频终态及两种打断；语音端到端延迟门禁保持不变。失败样本保留并明确归因，
服务重启期间的测试不作为新包验收证据。
